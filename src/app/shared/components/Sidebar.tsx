...
export default function Sidebar() {
    const { icons, loading, state } = useSidebar();
    return (
        <div className="h-full w-[60px] hover:w-[256px] hover:pl-2 pl-4 overflow-hidden p-2 transition-all duration-300 ease-in-out bg-gray-800 gap-4 flex flex-col">
            <nav className="flex flex-col">
                <ul className="flex flex-col gap-2">
                    {loading ? (
                        <Loader />
                    ) : state && state.sidebar.map((item: any) => (
                        <li key={item.label} className={(item.children && item.children.length > 0 ? "hover:h-30 hover:bg-gray-500" : "") + " flex flex-col rounded-xl min-h-6 h-10 overflow-hidden transition-all duration-300 ease-in-out"}>
